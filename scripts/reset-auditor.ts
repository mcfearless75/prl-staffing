 ...
 * Requires DATABASE_URL and AUDITOR_JWT_SECRET (or AUTH_SECRET) in your
 * environment — the secret must match the one deployed on Railway so the
 * signed link verifies on redemption.
 */

import { PrismaClient } from "@prisma/client";
import bcrypt from "bcryptjs";
import crypto from "crypto";
import { signAuditorLinkToken } from "../src/lib/auditor-link";
...
async function main() {
  ...
  const link = `${BASE_URL}/auditor/link/${signAuditorLinkToken(auditor.loginToken!)}`;
  ...
  console.log("    The link expires in 7 days and can be regenerated by re-running this script.\n");
}
