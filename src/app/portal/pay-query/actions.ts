 ...
 * The pay query record is already committed before this runs — a failed or
 * skipped send (no RESEND_API_KEY) must never block query submission; the
 * staff Pay Queries hub is the source of truth regardless of email delivery.
 */
async function notifyStaffOfPayQuery(query: {
  ...
  const fromEmail =
    process.env.EMAIL_FROM || "PRL Site Solutions <noreply@prlsitesolutions.online>";
  ...
  try {
    ...
    console.error("Portal pay-query staff notification failed:", error);
  }
}
